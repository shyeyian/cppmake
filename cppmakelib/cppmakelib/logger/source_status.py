...
def __init__(self):
    try:
        self._content = json.load(open(f"binary/{config.type}/cache/source_status.json", 'r'))
    except:
        self._content = {}
...
def __exit__(self):
    if len(self._content) > 0:
        ...
        json.dump(self._content, open(f"binary/{config.type}/cache/source_status.json", 'w'), indent=4)
